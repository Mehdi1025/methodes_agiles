<x-app-layout>
    ...
    <div class="min-h-screen bg-slate-50">
        <div class="max-w-2xl mx-auto px-4 sm:px-6 py-8">
            <div class="mb-6">
                <a href="{{ route('clients.show', $client) }}" class="inline-flex items-center gap-2 text-sm font-medium text-slate-500 hover:text-indigo-600 transition-colors">
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
                <div class="px-6 py-5 border-b border-slate-100">
                    <h1 class="text-xl font-bold text-slate-900">Modifier {{ $client->prenom }} {{ $client->nom }}</h1>
                    <p class="text-sm text-slate-500 mt-0.5">Mettez à jour les informations du client</p>
                </div>
                ...
                <form action="{{ route('clients.update', $client) }}" method="POST" class="p-6 space-y-6">
                    ...
                    <div>
                        ...
                        <input type="email" name="email" id="email" value="{{ old('email', $client->email) }}" required
                               class="w-full rounded-xl border border-slate-200 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 @error('email') border-red-300 @enderror">
                    ...
                    <div>
                        ...
                        <textarea name="adresse_livraison" id="adresse_livraison" rows="3" required
                                  class="w-full rounded-xl border border-slate-200 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 @error('adresse_livraison') border-red-300 @enderror">{{ old('adresse_livraison', $client->adresse_livraison) }}</textarea>
